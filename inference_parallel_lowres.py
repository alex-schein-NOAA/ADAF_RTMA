...
from models.encdec_lowres import * #build_model
...
# python inference_parallel_lowres.py \
...
def get_args():
    ...
    parser.add_argument("--hold_out_obs", type=str, default=False) #None
    parser.add_argument("--hold_out_obs_ratio", type=float, default=0.0) #None
...
def west_seam(params):
    ...
    """
    if not getattr(params, "mask_outside_grid184", False):
        return 0
...
def write_output_file(
    ...
    with xr.open_dataset(input_file, engine="netcdf4") as ds_in:
        ds_out, pred_analysis, pred_residual = crop_to_shared_domain(ds_in, pred_analysis, pred_residual)
        ...
        for i, var_name in enumerate(analysis_names):
            attrs = {
                ...
                "source": "ADAF_RTMA EncDecLowRes inference",
    ...
    with torch.no_grad():
        for batch_inputs, batch_hrrr, batch_obs_mask, batch_paths in loader:
            ...
            if params.learn_residual:
                pred_analysis = reverse_norm(pred_norm + hrrr_norm, hrrr_vmin, hrrr_vmax,
                                             channel_axis=1)
            else:

...
def load_stations_into_dataframe_and_clean_with_id(path, 
                                                 ADAF_CHANNELS=None, 
                                                 GOOD_QM=None, 
                                                 QM_FILLVALUE=None, 
    ...
    with h5py.File(path, "r") as f:
        ...
        # Look for dataProviderOrigin. Fallback to "METAR" if the field is missing (as is the case with ioda_adpsfc files)
        if "MetaData/dataProviderOrigin" in f:
            d["dataProviderOrigin"] = f["MetaData/dataProviderOrigin"][:].astype(str)
        else:
            # Creates an array of "METAR" matching the exact length of the coordinate data
            d["dataProviderOrigin"] = np.full(d["lat"].shape, "METAR", dtype=object)
            
        d["stationIdentification"] = f["MetaData/stationIdentification"][:].astype(str)
    ...
    # The new metadata string columns will be carried through automatically via .first()
    ...
    # Keeps rows if the QM is in GOOD_QM; if the QM is NaN then it's dropped
...
def load_mesonet_daily_uselist(mesonet_uselist_file, provider_uselist):
    column_names = [
        "name", "good_W", "good_T", "good_Td", "net",
        "N_T", "avg_T", "bias_T", "std_T",
        "N_S", "avg_S", "bias_S", "std_S",
        "N_DIR", "bias_DIR", "std_DIR", "std_W",
        "N_Td", "avg_Td", "bias_Td", "std_Td",
        "first_date", "first_time", "last_date", "last_time"
    ]
    
    df_uselist = pd.read_csv(mesonet_uselist_file,
                             sep=r"\s+", 
                             comment=";", 
                             header=None, 
                             names=column_names)

    # Recombine the split date and time columns into single datetime objects
    df_uselist["first_datetime"] = pd.to_datetime(df_uselist["first_date"] + " " + df_uselist["first_time"])
    df_uselist["last_datetime"] = pd.to_datetime(df_uselist["last_date"] + " " + df_uselist["last_time"])
    
    # Drop the redundant separate date/time columns
    df_uselist = df_uselist.drop(columns=["first_date", "first_time", "last_date", "last_time"])
    df_uselist = df_uselist[df_uselist["net"].isin(provider_uselist)].reset_index(drop=True)
...
def keep_closest_to_hour_per_location_with_time_threshold(df_input, 
                                                          time_col='OBS_TIMESTAMP', 
                                                          lat_col='lat', lon_col='lon', 
                                                          threshold_mins=30, past_obs_only=True):
    """
    Filters a DataFrame to keep the point closest to each hour, calculated independently for each unique geographic coordinate.

    If past_obs_only=True (default) then only observations from BEFORE each hour will be considered. 
    If False, then observations after the top of the hour will also be considered.
    
    Returns a dataframe with OBS_TIMESTAMP not yet rounded (or ceilinged) to the nearest hour.
    Should be applied to a dataframe that has already undergone regridding via assign_closest_with_threshold.
    """
    df = df_input.copy() 
    ...
    df['time_diff'] = (df[time_col] - df['target_hour']).abs()
    ...
    ## Priority 0 for METAR, Priority 1 for all other data providers
    if 'dataProviderOrigin' in df.columns:
        df['_src_priority'] = (df['dataProviderOrigin'].astype(str).str.upper() != 'METAR').astype(int)
    else:
        df['_src_priority'] = 0

    ## Sort by coordinates, target hour, METAR preference, then time difference.
    ## Drop duplicates keeps the first row (METAR first, then closest in time).
    df_filtered = (
        df.sort_values(by=[lat_col, lon_col, 'target_hour', '_src_priority', 'time_diff'])
          .drop_duplicates(subset=[lat_col, lon_col, 'target_hour'], keep='first')
    )

    # Clean up helper columns and sort
    return df_filtered.drop(columns=['target_hour', 'time_diff', '_src_priority']).sort_values(by=[time_col, lat_col])
    
###################

def filter_obs_by_temporal_completeness(df_input, OBS_TIME_WINDOW, obs_cols = ['sta_t', 'sta_q', 'sta_u10', 'sta_v10'], precision=6):
    """
    ...
    for col in obs_cols:
        # Count valid (non-NaN) entries per location
        valid_count = df.groupby(['lat', 'lon'])[col].transform('count')
        
        # Mask entire variable as NaN if valid count is less than OBS_TIME_WINDOW
        df.loc[valid_count < OBS_TIME_WINDOW, col] = np.nan
    
    # Drop rows where all 4 observation variables became NaN
    df_dropped = df.dropna(subset=obs_cols, how='all')
...
def reject_out_of_bounds_obs(df_input, dict_var_ranges=None):
    """
    Performs QC, setting any point where t, q, u10, or v10 is outside the acceptable range to NaN.
    Existing NaN values remain untouched.
    """
    df = df_input.copy()
    ...
    for col, (min_val, max_val) in dict_var_ranges.items():
        if col in df.columns:
            # Identify values strictly outside bounds
            # Comparison against NaN returns False, so NaNs are preserved automatically
...
def min_max_norm_ignore_extreme_keep_nan_sta_df(df_input, stats_path="stats.csv"):
    """Separate version of min_max_norm to apply to dataframes.

    Scales target columns to [-1, 1] based on external stats, clips extreme
    values outside the range, and preserves NaNs.
    """
    df = df_input.copy()
    ...
    for col in target_cols:
        ...
        if col in df.columns and col in stats.index:
            ...
            df[col] = (df[col] - vmin) * (2.0 / (vmax - vmin)) - 1.0

            # Clip values outside [-1, 1] (automatically preserves NaNs)
...
def assemble_station_dataset(df_obs,
                             lats_2d, lons_2d,
                             analysis_time,
                             time_col="OBS_TIMESTAMP",
                             lat_col="lat", lon_col="lon",
                             precision=6):
    """Takes in the dataframe of station data and returns an xarray Dataset with 3D observation variables (sta_*) and 2D source/validity masks (obs_mask_*) where:
        0 = No valid observation
        1 = Non-METAR observation (i.e. Mesonet)
        2 = METAR observation
    """
    ...
    # Extract coordinates for KDTree mapping
    ...
    # List of metadata/indexing columns to ignore when building observation variables
    ignore_cols = [lat_col, lon_col, "obs_time_window","dataProviderOrigin"]

    # Step 1: Create 3D arrays for observation columns (sta_t, sta_q, etc.)
    ...
    # Build initial xarray Dataset containing sta_* variables
    ds = xr.Dataset(
    ...
    # Map dataProviderOrigin to numeric source codes (2 = METAR, 1 = non-METAR)
    source_codes = np.where(df["dataProviderOrigin"].astype(str).str.strip().str.upper()=="METAR",2,1,).astype("int16")
    
    # Calculate per-variable 2D masks encoding source classification (0, 1, or 2)
    masks = {}
    for var in list(ds.data_vars):
        clean_var_name = (var.replace("sta_", "") if var.startswith("sta_") else var)
        mask_name = f"obs_mask_{clean_var_name}"

        # Identify rows in df that have a valid (non-NaN) observation for THIS specific variable
        valid_rows = df[var].notna().values

        y_valid = y_indices[valid_rows]
        x_valid = x_indices[valid_rows]
        codes_valid = source_codes[valid_rows]

        # Use np.maximum.at so METAR (2) takes precedence over non-METAR (1) deterministically
        source_grid_2d = np.zeros(lats_2d.shape, dtype="int16")
        if len(codes_valid) > 0:
            np.maximum.at(source_grid_2d, (y_valid, x_valid), codes_valid)

        # Check time-window completeness for this variable (1 if valid across window, 0 if not)
        time_completeness = (ds[var].notnull().all(dim="obs_time_window").values.astype("int16"))

        # Final 2D mask (preserves 0, 1, or 2)
        final_mask_2d = source_grid_2d * time_completeness

        # Store as DataArray with flag attributes
        masks[mask_name] = xr.DataArray(
            final_mask_2d,
            dims=("y", "x"),
            attrs={
                "flag_values": [0, 1, 2],
                "flag_meanings": "None Mesonet METAR",
            },
        )

    valid_time_ns = np.datetime64(analysis_time, "ns")

    # Attach masks, replace remaining NaNs with 0, and assign valid_time
    ds_final = (
        ds.assign(**masks)
        .fillna(0)  # Replaces NaNs in sta_* variables with 0
        .assign_coords(valid_time=valid_time_ns)
    )

...
class LayerScale2d(nn.Module):
    ...
    held-out temperature error equals HRRR's looks like, i.e. bad
    """
...
def _swin_stage(stage, f):
    """
    Converts from RSTB size (B, H*W, C) to image size (B, C, H, W).
    """
    b, c, h, w = f.shape
...
def _norm_act(dim, kind="layer", groups=8):
    if kind == "layer":
        ...
        raise ValueError(f"stem_norm must be ''layer'' or ''group'' or ''none'' (got {kind})")
...
class LowResEncDec(nn.Module):
    """
    Swin body at 1/downscale resolution, convolutions everywhere else.

    Same as EncDec -- (B, in_chans, H, W) -> (B, out_chans, H, W), residual vs HRRR -- but attention runs *only* on the downsampled grid. 
    """
    ...
    def __init__(self, params, norm_layer=nn.LayerNorm, **kwargs):
        ...
        self.upscale = params.upscale
        if self.upscale != 1:
            raise ValueError(f"lowres: upscale must be 1 (the network has no super-resolution stage; got {self.upscale})")
        self.mean = torch.zeros(1, 1, 1, 1)
        ...
        # The body sees H/downscale, and its windows must tile it -> H must be a multiple of downscale * window_size. 
        # 1356 and 2294 are multiples of neither, so the image must be padded
        ...
        # (H, W) order, matching the x_size forward() threads through 
